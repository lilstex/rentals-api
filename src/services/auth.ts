...
class AuthService {
  ...
  async welcomeText(): Promise<{ status: boolean, message: string, data?: any }> {
    try {
      ...
      return Util.handleError(error, 'welcome');
    }
  }
  ...
  async registerAccount(params: any): Promise<{ status: boolean, message: string, data?: any }> {
    try {
      ...
      //generate email code
      const emailVerificationCode = Util.generateOtpCode();
      ...
      // Serialize user data
      const userData = Util.serializeUserData(user);
      ...
      return Util.handleError(error, 'registerAccount');
    }
  }
    ...
    try {
      ...
      if (!user.isAccountVerified) {
        return {
          ...
          data: Util.serializeUserData(user)
        };
      }
      ...
      return Util.handleError(error, 'userLogin');
    }
  }
...
//         // Serialize user data
//         const userData = Util.serializeUserData(accountToVerify);
...
//         const resetToken = Util.generatePasswordToken();
...
//       data: Util.serializeUserData(account)
//     };
//   } catch (error) {
//     console.log(error);
//     return {
